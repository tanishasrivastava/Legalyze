...
import { FileText, Shield, Users, Upload, TrendingUp } from "lucide-react";
import DashboardLayout from "./DashboardLayout";
import "./Dashboard.css";

/* ─── FIXED: USE AI BACKEND ─── */
    ...
    const fetchDashboard = async () => {
      try {
        const res = await axios.get(
          `${AI_API}/api/user/dashboard-data/${email}`
        );
        setData(res.data);
      } catch (err) {
          ...
          <div className="contract-list">
            {data.recent_contracts.length > 0 ? (
              data.recent_contracts.map((contract) => (
                ...
                >
                  ...
                  <div className="contract-right">
                    <span className={`badge ${contract.risk.toLowerCase()}`}>
                      {contract.risk} Risk
                    </span>
              ...
              <div className="empty-state">
                No contracts found. Start by uploading one!
              </div>
            )}
          </div>
        </div>
